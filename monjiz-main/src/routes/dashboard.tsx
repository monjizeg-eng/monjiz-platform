...
function Dashboard() {
    ...
    (async () => {
      ...
      if (!admin) {
        try {
          ...
          setFreelancer(freelancerData as Freelancer);
          const projectsData = await listProjectsByFreelancer((freelancerData as Freelancer).id);
  ...
  if (!isAdmin && !freelancer) return (
    <Shell>
      <div className="container-mz py-20 text-center">
        <h1 className="text-2xl font-bold mb-3">Freelancer dashboard</h1>
        <p className="text-muted-foreground mb-4">This dashboard is for freelancers only. Create or complete your profile to access freelancer tools.</p>
        <Link to="/signup" className="inline-flex items-center justify-center px-5 py-3 bg-primary text-primary-foreground rounded-md hover:opacity-90">
      ...
      <Shell>
        <main className="container-mz py-16 flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
            ...
            <Link to="/admin" className="border border-border bg-primary text-white p-6 rounded-xl flex flex-col justify-between hover:bg-primary/95 transition">
              <div>
    ...
    <Shell>
      <main className="container-mz py-16 flex-1">
        <div className="mb-10">
          ...
          <p className="text-muted-foreground">This is your freelancer dashboard. Use the links below to manage your profile and active work.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Link to="/freelancer/$id" params={{ id: freelancer.id }} className="block border border-border bg-background p-6 rounded-xl hover:shadow-lg transition">
